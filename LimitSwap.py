...
parser.add_argument("-x", "--list_dex", action='store_true', help="The supported list of exchanges")
...
with open("./exchanges.json", ) as js_file:
    f = jsmin(js_file.read())
...
# List Dex
if command_line_args.list_dex == True: list_exchanges(dex_file)

# Load abi files
directory = './abi/'
filename = dex_settings['ROUTERABI']
file_path = os.path.join(directory, filename)
with open(file_path) as json_file:
    routerAbi = json.load(json_file)

directory = './abi/'
filename = dex_settings['FACTORYABI']
file_path = os.path.join(directory, filename)
with open(file_path) as json_file:
...
if settings['USECUSTOMNODE'] == "true":
    ...
    my_provider = dex_settings['MY_PROVIDER']
...
if int(dex_settings['CHAINID']) is not chain_id:
    printt_err('The specified CUSTOMNODE is not an', dex_settings['CHAIN'])
    exit(1)
printt(dex_settings['CHAIN'], 'Connected =', client.isConnected())
printt('Loading ' + dex_settings['NAME'] + ' v' + dex_settings['VERSION'] + ' Smart Contracts...')
if (dex_settings['DEBUG']) == 'True': printt_info('This DEX has not been tested, be careful, test with small amounts!')

routerAddress = Web3.toChecksumAddress(dex_settings['ROUTERADDRESS'])
factoryAddress = Web3.toChecksumAddress(dex_settings['FACTORYADDRESS'])
...
weth = Web3.toChecksumAddress(dex_settings['WETH'])
base_symbol = dex_settings['BASE_SYMBOL']
rugdocchain = dex_settings['RUGDOCCHAIN']
modified = dex_settings['MODIFIED']
settings['_EXCHANGE_BASE_SYMBOL'] = dex_settings['BASE_SYMBOL']
settings['_STABLE_BASES'] = dex_settings['STABLE_BASES']
